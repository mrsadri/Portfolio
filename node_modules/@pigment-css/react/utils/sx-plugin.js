...
var transform = require('@wyw-in-js/transform');
...
function validateObjectKey(keyPath, parentCall) {
  ...
  const identifiers = transform.findIdentifiers([keyPath]);
  ...
  properties.forEach((property) => {
    if (property.isObjectProperty()) {
      ...
      if (value.isObjectExpression()) {
        ...
        const identifiers = transform.findIdentifiers([value], "reference");
        ...
        if (localIdentifiers.length) {
          ...
          const fnBody = types.arrayExpression([types.cloneNode(value.node), types.booleanLiteral(unitLess)]);
          const arrowFn = types.arrowFunctionExpression([], types.stringLiteral(JSON.stringify(fnBody)));
          value.replaceWith(arrowFn);
        }
      }
    } else if (property.isSpreadElement()) {
      const identifiers = transform.findIdentifiers([property.get("argument")]);
...
function replaceNodePath(expressionPath, namePath, importName, t, tagNamePath) {
  const sxIdentifier = helperModuleImports.addNamed(namePath, importName, "@pigment-css/react");
...
var babelPlugin = helperPluginUtils.declare((api, { propName = "sx", importName = "sx" }) => {
  api.assertVersion(7);
  const { types: t } = api;
  return {
...
//# sourceMappingURL=sx-plugin.js.map
//# sourceMappingURL=sx-plugin.js.map