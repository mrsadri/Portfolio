...
function getSerializer(includeRtl) {
  if (!includeRtl) {
    return stylis.middleware([globalSelector, stylis.stringify]);
  }
  return stylis.middleware([globalSelector, rtlPlugin__default.default, stylis.stringify]);
}
getSerializer();
getSerializer(true);
...
var GlobalCssProcessor = class extends chunkIWNJMGWX_js.BaseProcessor {
  ...
  generateArtifacts(styleObjOrTaggged, ...args) {
    ...
    const { styles: cssText } = serialize.serializeStyles(
...
exports.GlobalCssProcessor = GlobalCssProcessor;
