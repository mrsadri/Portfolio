...
function getProps(paths) {
  ...
  paths.forEach((attr) => {
    if (isSxProp(attr)) {
      ...
      if (value.isJSXExpressionContainer()) {
        props.push(
          core.types.objectProperty(
            core.types.identifier(attr.node.name.name),
      ...
      props.push(
        core.types.objectProperty(
          core.types.identifier(attr.node.key.name),
          attr.node.value
        )
      );
    }
...
function spreadSxProp(tagPath) {
  ...
  if (sxPath) {
    ...
    if ("node" in expression) {
      if (target.isObjectExpression()) {
        spreadSxNode = core.types.spreadElement(expression.node);
        target.node.properties.push(spreadSxNode);
      }
      if (target.isJSXOpeningElement() && expression.isJSXExpressionContainer()) {
        spreadSxNode = core.types.jsxSpreadAttribute(expression.node.expression);
        target.node.attributes.push(spreadSxNode);
      }
    }
    sxPath.remove();
  }
  tagPath.node.arguments.push(core.types.objectExpression(props));
  if ((spreadSxNode == null ? void 0 : spreadSxNode.argument.type) === "ArrayExpression") {
    spreadSxNode.argument = core.types.callExpression(tagPath.node.callee, [
      spreadSxNode.argument,
      core.types.objectExpression(props)
    ]);
  }
  let arrayPath = null;
...
var SxProcessor = class extends chunkIWNJMGWX_js.BaseProcessor {
  constructor(params, ...args) {
    ...
    processorUtils.validateParams(params, ["callee", "call"], "Invalid usage of sx call.");
  ...
  build(values) {
    ...
    if (elementClassExpression.kind === shared.ValueType.LAZY) {
    ...
    let cssText = "";
    if (sxStyle.kind === shared.ValueType.CONST) {
  ...
  processCss(styleObjOrFn, expressionValue) {
    ...
    return chunk2ZKSYVEZ_js.processCssObject(styleObj, themeArgs, false);
  }
};

exports.SxProcessor = SxProcessor;
